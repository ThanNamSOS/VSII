...
@Component
public class WebserviceAddScheduler extends BaseScheduler implements FileChangeListener {
    ...
    public void onChange(Set<ChangedFiles> changeSet) {
        ...
        if (!CollectionUtils.isEmpty(lstFolder)) {
            LOGGER.info("Webservice Add start at: " + LocalDateTime.now());
            if (jmsUtils == null) {
            ...
            String webServiceAddError = fileService.buildFolderPath(propertiesConfig.getFolderError(),
                    propertiesConfig.getFolderWebserviceADD());
            File fileError = null;
            for (File file : lstFolder) {
                ...
                if (error == null) {
                    try {
                        ...
                        fileError = new File(fileService.buildFolderPath(webServiceAddError, file.getName()));
            ...
            LOGGER.info("Webservice Add and at: " + LocalDateTime.now());
        }
    }
}
