...
@Component
public class ScanNewScheduler extends BaseScheduler implements FileChangeListener {
    private static final Logger LOGGER = LoggerFactory.getLogger(ScanNewScheduler.class);
    ...
    public void onChange(Set<ChangedFiles> changeSet) {
        ...
        if (!CollectionUtils.isEmpty(lstFolder)) {
            LOGGER.info("Scan New start at: " + LocalDateTime.now());
            if (jmsUtils == null) {
            ...
            String ScanNewError = fileService.buildFolderPath(propertiesConfig.getFolderError(),
                    propertiesConfig.getFolderScanNCPPX());
            File fileError = null;
            for (File file : lstFolder) {
                ...
                if (error == null) {
                    try {
                        ...
                        fileError = new File(fileService.buildFolderPath(ScanNewError, file.getName()));
            ...
            LOGGER.info("Scan New and at: " + LocalDateTime.now());
        }
    }
}