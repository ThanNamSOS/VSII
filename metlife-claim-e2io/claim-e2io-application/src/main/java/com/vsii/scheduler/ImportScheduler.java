...
@Component
public class ImportScheduler extends BaseScheduler implements FileChangeListener {
    ...
    public void onChange(Set<ChangedFiles> changeSet) {
        ...
        if (!CollectionUtils.isEmpty(lstFolder)) {
            ...
            String importError = fileService.buildFolderPath(propertiesConfig.getFolderError(),
                    propertiesConfig.getFolderImportPPX());
            File fileError = null;
            for (File file : lstFolder) {
                ...
                if (error == null) {
                    try {
                        ...
                        fileError = new File(fileService.buildFolderPath(importError, file.getName()));
